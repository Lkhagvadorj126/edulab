...
export default function NavH() {
  ...
  const dynamicLinks = Object.keys(LESSONS_CONFIG).map((key) => ({
    href: `/chemistry/${key}`, // ЭНД ЗАСАВ: chemistry хавтас руу заана
    label: LESSONS_CONFIG[key].page.title.split(" ")[0],
  }));

  const links = [{ href: "/indexH", label: "Нүүр" }, ...dynamicLinks];
    ...
    <nav className="w-full mb-10 relative z-50">
      <div className="bg-white border border-slate-100 rounded-[1.5rem] p-2 shadow-sm flex items-center justify-between w-full">
        ...
        <button
          ...
          className="md:hidden p-3 rounded-xl text-[#312C85]"
        >
        ...
        <div className="hidden md:flex items-center justify-between w-full gap-1">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`${baseClass} ${desktopClass} ${pathname === link.href ? activeClass : normalClass}`}
            >
              {link.label}
            </Link>
          ))}
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-3 p-3 bg-white border border-slate-100 rounded-[2rem] shadow-2xl md:hidden flex flex-col">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              onClick={() => setIsOpen(false)}
              className={`${baseClass} ${mobileClass} ${pathname === link.href ? activeClass : normalClass}`}
            >
              {link.label}
            </Link>
          ))}
